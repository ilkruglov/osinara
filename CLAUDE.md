...
и не попадает в источники тихой проверки памяти. Ограничитель `TELEGRAM_BOT_EXCHANGE_LIMIT`:
после четырёх подряд ходов от ботов без сообщения человека Мия ботам не отвечает
(`AGENT_TELEGRAM_BOT_EXCHANGE_LIMIT_REACHED`).
