...
приходит с авторизацией этого ответа. Пакет в статусе `running` не имеет аренды и ждёт только
собственный ход, поэтому у брошенного хода есть последний выход по времени: припаркованный ход не
трогают, его видно по `pending_operation` сессии, а после срока судьбу пакета решает провенанс
`eve:<session>:<turn>`. Записавший память ход засчитывается состоявшимся, потому что повтор создал
бы дубликаты. Ничего не записавший отдаёт источники обратно в непроверенный хвост, но только пока
за ним никто не стоит: следующие пакеты цепляются за застрявшую голову, и удаление головы оставило
бы их недостижимыми от курсора. При наследнике голова получает терминальный `skipped` — строка
остаётся, курсор проходит через неё, источники отпускаются, а владелец получает предупреждение, что
этих сообщений не увидит уже ни один проход. Сессию фоновой проверки закрывает тот же проход:
`retireAbandonedTasks` её вида не знает, а живые источники держали бы журнал группы от обрезки.
...
Если имя лишь упомянули, а обращаются к другому, промпт требует пустой ответ: харнесс не шлёт
ничего (`completedTelegramOutput` → null), реакция или «молчу» текстом не считаются молчанием.
