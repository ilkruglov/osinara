...
не возобновляется до следующего постороннего сообщения. Страховка в ingress: после
`approval.settled` без возобновления хода в течение 15 с callback считается обработанным
(`AGENT_TELEGRAM_APPROVAL_BOUNDARY_ASSUMED`), а доставка без единого события за 30 с считается
принятой (`AGENT_TELEGRAM_DISPATCH_BOUNDARY_ASSUMED`): сообщение, пришедшее в чат, пока сессия
ждёт кнопку, Eve молча откладывает до ответа и не начинает ход. Иначе один drain на бота
замораживал все чаты.
