...
Лимиты живут в `reminder-config.ts` и считаются по живым записям (`active`, `leased`, `paused`):
`GROUP_REMINDER_MAX_PER_AUTHOR` на одного Telegram-автора в чате и `GROUP_REMINDER_MAX_PER_CHAT` на
чат. Подсчёт и вставка идут в одной транзакции под advisory lock по группе, иначе двое одновременно
...
Предстоящие напоминания чата видит любой участник, приостановленные только их автор, а изменить,
приостановить и возобновить можно лишь своё. Удаление отличается: чужую запись разрешено удалить
после того, как live-проверка `getChatMember` подтвердила, что автора в чате больше нет. Иначе
ушедший автор оставлял бы неудаляемое напоминание, которое пишет в чат вечно и держит один из
тридцати слотов. Неопределённый ответ провайдера удаление отклоняет: считать его уходом означало бы
удалять запись присутствующего автора по сбою. Telegram гарантирует ответ по чужому пользователю
только администратору чата, поэтому в группе без админских прав бота эта проверка может не дать
ответа никогда. Дверь, которая работает всегда, у владельца: `manage_telegram_group` с
`list_reminders` и `delete_reminder` показывает и удаляет напоминания подключённой внешней группы из
личного чата, без проверки автора и с HITL. Доверенный путь до чужой зоны не достаёт:
`requireReminderMutationAccess` отклоняет scope `group`, а личный список его не показывает. Групповая проактивная доставка
