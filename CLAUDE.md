...
Внутри внешней группы бот является обычным участником, а не урезанным каналом: порог срабатывания
тот же (упоминание, ответ агенту или имя в тексте), набор инструментов тот же, что у человека в этой
группе, и групповое напоминание он ставит под своим Telegram id. Ограничения на число реплик подряд
нет сознательно, поэтому стоп-краном остаётся тумблер в @BotFather или режим чата.

Отличается только то, для чего у бота физически нет носителя. Подтверждение действия адресуется
человеку кнопкой, поэтому `input.requested` и `authorization.required` отклоняют актора без учётной
записи (`accountlessActorApprovalError`), иначе сессия ждала бы адресата, которого нет.
`verified_profile_view` строится вокруг человеческой identity, поэтому на ходе бота профиля нет, а
retrieved memory остаётся. Сообщение бота не становится свидетельством памяти и не меняет настройку
общения: и то и другое хранится за конкретным человеком.
