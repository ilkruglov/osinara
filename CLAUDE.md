...
С 9 сентября 2026 основная модель это превью V4.1 flash по алиасу
`deepseek-v4.1-flash-expires-on-0910` (в списке моделей его нет, по прямому id отвечает; владелец
счёл его остроумнее v4 в слепом отчёте на 30 ходах: короче, вдвое реже схема «X. А Y», 5 с против
10 с). Алиас умирает 10 сентября, поэтому транспорт умеет `fallbackModelId`: на ответ DeepSeek
«supported API model names are …» для настроенного id запрос один раз уходит с запасной моделью
(`deepseek-v4-flash`) и пишется `AGENT_MODEL_ID_RETIRED`. Прод НЕ читает `config/agent-model-providers.json`
...
`deepseek-flash`, `fallbackModelId` убран; `deepseek-flash` мультимодальный, проверено картинкой.
Сверка с карточкой DeepSeek-V4.1-Flash на Hugging Face (10 сентября 2026): рекомендованы
`temperature` 1.0 и `top_p` 0.95 или 1.0, окно 1M, `max_tokens` от 256K. Хостед API уже отдаёт
`temperature: 1.0` и `top_p: 1.0` по умолчанию, мы их не задаём и не режем, так что совпадаем.
Обещанный карточкой непрерывный effort числом 1–100 хостед API не принимает
(`reasoning.effort: expected value`), это про открытые веса и их библиотеку encoding; у нас
остаётся документированный enum none/low/high/max. `max_output_tokens` 300000 API принимает, но в
конфиге оставлено 128000: при окне компакции 160k больше не нужно.
Effort основной модели с 10 сентября 2026 `high` (был `max`): эвал харнесом на 30 ходах Ft86 по два
сэмпла дал медиану 3,8 с против 24,1 с, 90-й процентиль 6,0 с против 89,4 с, у max 7 обрывов API и
23 ответа дольше 30 с из 53; на проде max давал 46 тысяч токенов рассуждения на вопрос про
кофейную гущу (4 минуты). Vision остаётся `high`, на картинках max рассуждал в 5–10 раз дольше
без выигрыша в ответе. Канал вырезает утёкшую разметку вызова инструмента
...
Припаркованные прогоны Workflow (10 сентября 2026). Сессия Eve живёт как прогон, который ждёт
сообщения на hook и потому навсегда остаётся `running`. Удаление хранилища отказывалось трогать
такой прогон (`AGENT_EVE_SESSION_STORAGE_ACTIVE`), а заявка ретенции пропускала строку с
`cleanup_error_code` и повторов не делала: 36 сессий стояли неудаляемыми с 3 сентября, держали 91
живой прогон и 223 МБ событий, и каждый старт агента вычитывал их журналы целиком (74 секунды и
около гигабайта чтений на деплой; видно по `AGENT_WORKFLOW_EVENT_LOG`). Теперь удаление само гасит
прогон, простоявший без изменений больше часа, раз приложение вывело этот разговор сутки назад;
hook блокирует удаление только с живым окном удержания (`token_retention_until > now()`), а пустое
окно Workflow и сам считает законченным. Неудачное удаление получает
`cleanup_retry_after = now() + SESSION_RETENTION_FAILURE_COOLDOWN_MINUTES` (миграция 101, отдельная
колонка: пара полей аренды связана check-ограничением) и повторяется следующим проходом, а не
паркуется до ручной правки. 91 осиротевший прогон погашен вручную 10 сентября.
...
«Разрешено всё» у Telegram означает только его собственный набор `ReactionTypeEmoji` (73 эмодзи,
`TELEGRAM_REACTION_EMOJI` в `telegram-message-reaction.ts`, снят с документации 9 сентября 2026):
на любой другой эмодзи Bot API отвечает `400 REACTION_INVALID`, и до fork-89 харнесс считал это
«реакции недоступны» и молчал (кошка на «молодец, хорошая кошка» в Ft86, три минуты тишины).
Теперь промпт перечисляет весь набор, `completedTelegramOutput` канонизирует эмодзи (без U+FE0F),
а эмодзи вне набора заменяет ближайшей разрешённой реакцией по настроению
(`nearestTelegramReactionEmoji`: кошки и нежность в 🥰, смех в 🤣, улыбки в 😁, сердца в ❤,
грусть в 😢, праздник в 🎉 и т.д., иначе 👍; лог `AGENT_TELEGRAM_REACTION_SUBSTITUTED`).
Текстом эмодзи не шлётся: по решению владельца жест это реакция, а не сообщение (fork-90 отменил
текстовый откат fork-89). Отказ Telegram 400/403 остаётся тихим, лог
`AGENT_TELEGRAM_REACTION_UNAVAILABLE` несёт эмодзи и `providerDescription`.
...
Автокат «Полный ответ» (`telegram-final-presentation.ts`, 8 сентября 2026, решение владельца):
триггеров два, длина больше 600 символов и блок кода или таблица. Upstream-критерии по строкам
(7), абзацам (2) и пунктам списка (5) убраны: они прятали десятистрочный стих и шесть коротких
пунктов. Лид снаружи ката остаётся только для прозы (первый блок до 240 символов и до двух
строк), столбик коротких строк сворачивается целиком, а не с разрывом между строфами. Мия
по-прежнему может свернуть блок сама (`<details>`), инструкция в `agent/instructions.md`
согласована с кодом.
...
Язык ответа только русский (10 сентября 2026, решение владельца; в тот же день Мия час говорила в
Ft86 по-украински уже с новым ядром, потому что тихая проверка сохранила «договорённость» с
Vadim (Akihabarai) отвечать ему по-украински, и запись подставлялась в каждый ход; запись снята
мягким удалением, промпт проверки теперь запрещает сохранять язык ответа в любом слоте, а ядро
говорит, что договорённость и запись памяти языка не меняют): чужой язык во входящем сообщении
не считается просьбой перейти на него, иностранный текст Мия пишет единственно как запрошенный
перевод, а имена, термины, ссылки, код и точные цитаты остаются в оригинале. Язык убран из того,
что меняют устойчивые настройки стиля, поэтому просьбу «отвечай по-английски» она не выполняет и
не сохраняет; перевод по просьбе делает всегда. Бюджеты ядра и общего промпта подняты до 12 000 и
35 400 символов.
...
Часы таймлайна (8 сентября 2026): метки времени записей и разделитель дня рендерятся в часовом
поясе хода (`currentTimeRepository.findTurnTimezone`: настройка отправителя из
`user_notification_settings`, иначе настройка владельца семьи, иначе UTC), разделитель называет
пояс: `-- 2026-09-08 Europe/Moscow (+03:00) --`. Тот же пояс идёт в `<current_time>`. Повод: Мия
в Ft86 цитировала «в 15:19» про сообщение 18:19 по Москве, потому что видела только UTC, а
таймзона ни у кого не была настроена. `PreparedTelegramGroupTurnContext.timelineTimezone` хранит
пояс для повторного рендера в memory-review ходе. Для пересказов за период протокол
`list_group_history` велит читать диапазон по from/to и излагать в порядке sequence: сводка дня
из обрывков сессии переставила эпизод 18:17 в «финал» (8 сентября 2026).
...
Патч журнала событий прогона (`scripts/eve-patches/event-log-cache.ts`, 10 сентября 2026): при
пробуждении припаркованного прогона `@workflow/core` перечитывает весь его журнал, а world-postgres
отвечает `select *`. Вызовов два: восстановление после сбоя просит события без нагрузки
(`resolveData: "none"`), а replay прогона просит их целиком, и кэшировать надо именно второй —
первая версия патча покрывала только первый и не срабатывала ни разу, что и показал трейс
`AGENT_WORKFLOW_EVENT_LOG` (0 строк на проде и в сквозном тесте). Сессия из 43 ходов
несла 585 событий и 9 МБ полезной нагрузки (в среднем 47 КБ на `hook_received` и 34 КБ на
`step_created`, это наш же контекст хода), и всё это читалось и разбиралось на каждое сообщение:
0,22 с запросов и большая часть 0,7 с процессорной паузы до вызова модели. Журнал только растёт,
поэтому прочитанный префикс держится в памяти процесса (`scripts/eve-runtime/event-log-cache.ts`),
сверяется с живым `count(*)` и дочитывается хвостом; меньшее число строк роняет кэш и читает всё
заново. Режимы разведены по ключу, чтобы вызывающий не получил чужую форму. Кэш ограничен
оценкой объёма (64 МБ всего, 24 МБ на прогон, до 12 прогонов, вытеснение по давности), оценка
берётся по выборке событий и не разворачивает двоичные payload'ы. События отдаются по ссылке:
`@workflow/core` только складывает их в свои массивы и не пишет внутрь события. Мимо кэша идёт
запрос с пагинацией, курсором или обратным порядком. Проекцию колонок вместо
кэша сделать нельзя: `stripEventDataRefs` при `resolveData: "none"` убирает лишь крупные ссылочные
поля, а метаданные внутри нагрузки остаются нужны, и схема события требует это поле у десяти типов
из тринадцати.
...
Материализация навыков (10 сентября 2026). Между резолверами навыков и блоком режима каждый ход
уходило 2–5 секунд: `dispatchDynamicSkillEvent` Eve пишет каждый динамический пакет заново на
каждом ходе, файл за файлом, без сравнения с тем, что уже лежит в контейнере, а внешняя группа с
`web_search` получает три пакета (два аналитика и `imagegen`) по пять файлов. Одна запись стоила
три операции Docker: exec на каталоги, `putArchive`, exec на перенос из staging (замер на проде:
exec 177 мс, putArchive 48 мс). Сделано: патч Eve пишет файлы пакета и сами пакеты параллельно
(`writeSkillPackageToSandbox`, `dynamic-skill-lifecycle.js`), а runner помнит, что уже записал в
этот запуск контейнера (`sandbox-write-memo.ts`) и пропускает повторную запись тех же байтов.
Ключ памяти это поколение контейнера (id плюс время старта, `docker-sandbox-container.ts`):
`$HOME` ограниченного sandbox лежит на tmpfs, перезапуск стирает навыки, и память по одной сессии
оставила бы `load_skill` без пакетов до ротации. Пропускаются только пакеты навыков; файл
workspace переписывается всегда, его никто больше не восстановит. Staging-каталог создаётся один
раз на запуск контейнера, а `mkdir` целевого каталога уехал в ту же команду, что и `mv`: одна
операция exec на запись вместо двух.

Трейсы базы (10 сентября 2026). Разбор задержки требовал включать
`log_min_duration_statement` на проде и ждать живого сообщения, поэтому обе базы теперь говорят
сами. Запрос от `DATABASE_SLOW_QUERY_MS` (150 мс) пишет `AGENT_DATABASE_SLOW_QUERY`
(`agent/lib/pool-trace.ts`, обёртка пула в `database.ts`), запрос базы workflow от того же порога
пишет `AGENT_WORKFLOW_SLOW_QUERY` (`scripts/eve-runtime/workflow-pool-trace.ts`, обёртка
единственного пула драйвера в `index.js` покрывает storage, очередь и streamer). В обоих случаях
логируется форма запроса, длительность и число строк, но никогда значения параметров: они несут
переписку семьи и содержимое памяти. Каждое пробуждение прогона пишет `AGENT_WORKFLOW_EVENT_LOG`
с числом переиспользованных и дочитанных событий, это живая проверка кэша журнала. Индексов не
хватало нигде: 10 сентября сверены `pg_stat_user_tables` и захваченный журнал запросов, все горячие
пути идут по индексам, а оставшиеся последовательные сканы приходятся на таблицы в несколько строк
и фоновые уборки.
