...
Язык ответа только русский (10 сентября 2026, решение владельца): чужой язык во входящем сообщении
...
пробуждении припаркованного прогона `@workflow/core` перечитывает весь его журнал
(`events.list` с `resolveData: "none"`), а world-postgres отвечает `select *`. Сессия из 43 ходов
...
до 24 прогонов), сверяется с живым `count(*)` и дочитывается хвостом; меньшее число строк роняет
кэш и читает всё заново. Кэшируется только полный список по возрастанию без полезной нагрузки:
запрос с пагинацией, курсором, обратным порядком или с данными идёт мимо. Проекцию колонок вместо
