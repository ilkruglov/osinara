...
доставки присоединяется к её Promise, новые доставки того же run/step сохраняются и сериализуются.
Это process-local барьер текущего однопроцессного runtime, не замена durable-ledger приложения и не
гарантия между репликами. Нативная сериализация Workflow и политика повторов модели сохраняются.
