...
Authored model context внешней группы не должен содержать длинное или короткое типографское тире и кавычки-ёлочки. Permanent core очищается в исходнике, собранные external mode blocks и grantable skill packages проходят явную authored-only нормализацию; пользовательские сообщения, история, память, файлы и tool data никогда не переписываются этой политикой.
