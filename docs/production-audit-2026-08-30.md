...
## Обновление 2026-09-05: восстановлен ошибочный индекс

Единственная задача индексации, остававшаяся failed с 2 августа, восстановлена точечно на production.
Причина воспроизведена: начало перекрывающегося фрагмента разрезало Unicode surrogate pair, и TEI
отклонял JSON с HTTP 400 (`lone leading surrogate`). Исходная запись не была повреждена.

Исправленная нарезка сохраняет целые кодовые точки и прежний контракт UTF-16 offsets. В 09:41 UTC
получены и атомарно сохранены 12 векторов по 384 измерения; SHA-256 текста до и после совпал,
`updated_at` исходной записи не изменился. Операция записана в audit, ошибочная задача удалена штатным
завершением. На момент проверки все 860 активных записей имели статус `indexed`.

Исправление нарезки внесено в код и покрыто тестами; постоянно работающий production worker на момент
восстановления ещё использовал прежний релиз. Для защиты будущих записей исправление нужно включить
в следующий штатный релиз. Остальные пункты этого аудита данной операцией не изменены.
