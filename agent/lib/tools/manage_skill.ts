...
const TOOL_DESCRIPTION = [
  ...
  `Markdown навыка без frontmatter, обязательные разделы: ${AUTHORED_SKILL_REQUIRED_SECTIONS.map((section) => `«${section}»`).join(", ")}; в шагах имена инструментов в обратных кавычках только из текущего режима. Лимиты: markdown ${AUTHORED_SKILL_LIMITS.markdownMaxCharacters} символов, до ${AUTHORED_SKILL_LIMITS.filesMax} файлов references/<имя>.md по ${AUTHORED_SKILL_LIMITS.fileMaxCharacters} символов, ${AUTHORED_SKILL_LIMITS.activeSkillsPerFamily} активных навыков на семью.`,
