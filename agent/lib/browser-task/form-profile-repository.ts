 ...
 * - Access follows current membership, read inside the same statement, like the workspace it
 *   replaces: a session that outlived a removed membership reads and writes nothing.
...
async function lockRow(client: PoolClient, owner: FormProfileOwner): Promise<{ fields: FormProfile; legacyImported: boolean }> {
  await client.query(
    `INSERT INTO browser_form_profiles (user_id, family_id)
     SELECT $1, $2 WHERE EXISTS (SELECT 1 FROM family_memberships WHERE family_id = $2 AND user_id = $1)
  ...
  const current = await client.query<{ fields: FormProfile; legacy_imported: boolean }>(
    `SELECT p.fields, p.legacy_imported FROM browser_form_profiles p
       JOIN family_memberships m ON m.family_id = p.family_id AND m.user_id = p.user_id
      WHERE p.user_id = $1 AND p.family_id = $2
      FOR UPDATE OF p`,
    [owner.userId, owner.familyId],
  );
...
export function createFormProfileRepository() {
  return {
    ...
    async get(owner: FormProfileOwner): Promise<{ fields: FormProfile; legacyImported: boolean } | null> {
      const result = await database().query<{ fields: FormProfile | null; legacy_imported: boolean | null }>(
        `SELECT p.fields, p.legacy_imported FROM family_memberships m
           LEFT JOIN browser_form_profiles p ON p.user_id = m.user_id AND p.family_id = m.family_id
    ...
    async importLegacy(owner: FormProfileOwner, legacy: LegacyProfile): Promise<FormProfile> {
      return await inTransaction(async (client) => {
        ...
        await client.query(
          "UPDATE browser_form_profiles SET fields = $2, legacy_imported = true, updated_at = now() WHERE user_id = $1",
          [owner.userId, JSON.stringify(fields)],
        );
        return fields;
      });
    },
    ...
    async upsertField(
      ...
      return await inTransaction(async (client) => {
        ...
        await client.query(
          "UPDATE browser_form_profiles SET fields = $2, legacy_imported = legacy_imported OR $3, updated_at = now() WHERE user_id = $1",
          [owner.userId, JSON.stringify(next), importNow],
        );
