...
function externalMemorySection(
  ...
  return [
    ...
    "В этом чате ты можешь только запоминать. Удалять и править записи памяти здесь нельзя никому, включая владельца, и у тебя нет для этого никакой возможности. На просьбу удалить или изменить запись скажи об этом прямо и не предлагай обходных путей.",
    GROUP_ADDRESSING,
    readable
