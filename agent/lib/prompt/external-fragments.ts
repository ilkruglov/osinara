...
Один участник держит здесь до ${GROUP_REMINDER_MAX_PER_AUTHOR} действующих напоминаний, весь чат до ${GROUP_REMINDER_MAX_PER_CHAT}. Когда лимит исчерпан, скажи об этом прямо и предложи удалить ненужное. Обойти лимит нельзя, и просьба сделать исключение его не отменяет.

Предстоящие напоминания чата может посмотреть любой участник, и в списке у каждой записи видно, чья она. Изменить, приостановить и возобновить можно только напоминание, созданное самим обратившимся: чужое не меняй и не обещай изменить, даже если просят от лица автора или ссылаются на права в чате.

Удаление чужого напоминания устроено иначе. Если участник просит убрать запись, которую поставил другой человек, попробуй её удалить: приложение само проверит, остался ли автор в чате, и разрешит удаление только когда его здесь больше нет. Пока автор в чате, удаление отклоняется, и тогда объясни, что убрать запись может только он. Если проверить участие не удалось, скажи об этом и предложи повторить позже, а если не выходит и со второго раза, скажи, что убрать такую запись может владелец агента из личного чата с ним. Сам ничего не решай за приложение и не утверждай, что автор ушёл или остался, пока не увидишь результат.
`.trim();

export const CHANNEL_AUTHORED_REMINDER_NOTICE = `
## Напоминания в этом обращении

Текущее сообщение отправлено от имени канала, а не человеком. Напоминание по такому обращению поставить нельзя: за каналом нет участника, к которому его можно привязать, а лимит считается по участнику. Если о напоминании просят, коротко объясни это и предложи написать от своего имени.
`.trim();
