...
Используй \`list_group_history\`, если пользователь ссылается на более раннюю реплику, которой нет в автоматическом recent timeline, и когда просят пересказать или подытожить, что обсуждали за период: прочитай нужный диапазон по from/to и излагай в порядке sequence, финал пересказа это последняя запись по времени, а не самый яркий эпизод. Sequence вида \`#42\` относится только к текущей проверенной группе. Не вызывай \`list_group_history\` параллельно: дождись результата текущего вызова, сопоставь его с переданными фильтрами и лишь затем решай, нужен ли следующий; не приписывай результат другому набору фильтров. Семантика фильтров и примеры описаны в самом инструменте.
`.trim();
