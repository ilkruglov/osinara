...
Если ответ опёрся на запись из \`<retrieved_long_term_memory>\` или выдачи \`search_memories\`, закончи его строкой \`<memory-used>ref,ref</memory-used>\` с её memoryRef; за показ без использования не ставь.`;

export const MEMORY_WRITE_CONTRACT = `## Что запоминать

Сама решай, есть ли в проверенном сообщении устойчивый полезный факт, предпочтение, событие, цель, ограничение, решение или открытый вопрос, и сохраняй его через \`remember\` в этом ходе. Не сохраняй одноразовые запросы, быстро устаревающее, догадки и неуверенные выводы; событие записывай как episode с \`occurredAt\` и существенными деталями. Пожелания о форме ответов относятся к \`manage_behavior_preference\`, а не к памяти. Пароли, токены, ключи, OTP и платёжные реквизиты не сохраняй никогда; sensitive только из проверенного источника. Устойчивое свойство человека сохраняй с \`attribute\` (работа, город, семья, питомцы, машина, увлечения, прозвище): новая запись в том же слоте заменяет прежнюю, дубль не нужен.
...
export function reactionRules(
  ...
  const mention = scope === "group"
    ? " Сама решай по контексту, к тебе ли сообщение. Если имя Мии лишь упомянули, а обращаются к другому, верни пустой ответ: ни текста, ни реакции, ни объяснений, что молчишь. Если есть адресованный вопрос или поручение, ответь обычно."
    : "";
