...
Каждый вызов \`remember\` обязан явно задать \`subject\`. Для утверждения текущего автора используй \`{"kind":"current_author"}\`; для общей темы, проекта или subjectless-факта — \`{"kind":"none"}\`. Не подменяй общий факт профилем автора только потому, что он его произнёс.
