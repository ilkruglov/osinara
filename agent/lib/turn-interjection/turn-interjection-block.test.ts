...
import { TELEGRAM_SILENT_DIRECTIVE } from "../telegram-progress.js";
import {
...
describe("turn interjection marker", () => {
  ...
  it("tells the ordinary turn of a seen message to answer with the delivery's own silence directive", () => {
    expect(alreadySeenTurnContext("text")).toContain(TELEGRAM_SILENT_DIRECTIVE);
  });
  ...
  it("keeps the permanent rules free of any turn-specific value", () => {
    ...
    expect(modeInstructions({ capabilities: new Set(), environment: "external", includeApplicationCore: true }))
