 ...
 * - `MEMORY_RELEASE_MIGRATIONS`: the only migrations that v0.11.0 may apply to that ledger.
...
describeWithDatabase("v0.10.1 production ledger upgrade to memory migrations 049-056", () => {
  ...
  it("applies only the renumbered memory release once and creates the R0-R7 schema", async () => {
    ...
    try {
      ...
      // The ledger delta proves the real runner skipped all shipped migrations and applied 049-056 once.
