...
`stonksMode: true` — ОДИН РАЗ покажи текст из поля `warning`, но не трактуй его
как отмену preview и подтверждения: граница Osinara требует `--confirm` всегда.
...
3. **Никакой автономной торговли по своей инициативе.** Не выставляй заявки
   в циклах, по расписанию или «по достижении цены» без пользователя. Для
    ...
    их тоже подтверждает пользователь. `stonksMode` не отменяет это правило в
    Osinara: каждая реальная или sandbox mutation требует явного подтверждения
    конкретной операции в текущем диалоге.
...
- В песочнице — тоже уместно (учим на безопасном), но короче. В stonks-режиме
  сохраняй тот же preview и явное подтверждение каждой сделки; системная граница
  Osinara имеет приоритет над настройкой CLI.
...
**Торговля (sandbox и full — с --confirm на каждую операцию; full дополнительно требует deployment gate; readonly — только чтение):**
...
- В режиме песочницы CLI печатает в stderr баннер «Режим песочницы» — упоминай в ответе, что данные виртуальные. В stonks-режиме баннер напоминает, что граница Osinara всё равно требует preview и `--confirm`.
