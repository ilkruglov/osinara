...
Для правки передай `images` с исходником из Telegram или workspace и укажи в prompt, что изменить
и что сохранить. Можно передать до четырёх исходников. Редактирование сейчас подключено через
Cloudflare: исходники уменьшаются до размера меньше 512×512, результат сохраняется новым файлом.
Перехода на обычную генерацию NeuralDeep при отказе правки нет. Результат отправляй отдельным
вызовом `send_workspace_image`, только после успешного сохранения.
...
| Задача | size |
| --- | --- |
| Открытка, аватар, карточка блюда | `1024x1024` (даёт 512×512) |
